...
- [Expert Evaluation (Likert Scale)](#-expert-evaluation-likert-scale)
- [Examples](#-examples)
...
## 🧑‍⚕️ Expert Evaluation (Likert Scale)

This Likert scale is designed for **blind expert rating** of the clinical quality of OMGs-generated MDT recommendations in ovarian cancer.

### Likert Rating Table (Expert Version, Chinese)

| Domain | Definition | Anchors (1–5) |
|---|---|---|
...
| **Personalization（个体化）** | 推荐方案基于患者关键临床与生物学特征形成差异化路径（治疗线别/铂状态、HRD/BRCA/MMR 等、ECOG/PS、合并症与治疗目标）。 | **5**：关键变量驱动；路径分层清晰一致。<br>**4**：个体化充分；少量次要因素未展开。<br>**3**：提及差异；对路径塑形有限。<br>**2**：通用建议为主；个体化较弱。<br>**1**：模板化输出。 |
| **Evidence（证据：强度与可追溯性）** | 关键结论有证据支持且可追溯：患者层面证据（检验/影像/病理/分子）与外部证据（指南/PMID/试验ID）；引用与主张需匹配。 | **5**：关键主张证据充分；引用准确可追溯。<br>**4**：证据总体充分；少量引用缺口。<br>**3**：证据链不完整；部分关键点支撑不足。<br>**2**：证据薄弱或引用不匹配。<br>**1**：缺乏可信证据或不可追溯。 |
| **Actionability（可执行性）** | 输出能够形成清晰的临床处置路径，涉及主要评估、核心治疗策略、调整触发条件三个环节。 | **5**：路径完整清晰；可直接执行。<br>**4**：基本可执行；少量操作细节欠缺。<br>**3**：方向正确；需补关键步骤后执行。<br>**2**：措施偏原则性；缺少明确行动要点。<br>**1**：不可执行。 |
| **Robustness（稳健性）** | 能识别不确定性与缺失信息，说明其对决策影响，并给出可操作的补救/风险控制方案（补检、复评、延后、替代、监测触发）。 | **5**：缺口识别充分；补救与风控可直接落实。<br>**4**：覆盖主要缺口；少量边界不足。<br>**3**：指出缺口；补救不够具体。<br>**2**：缺口识别不足；风控措施弱。<br>**1**：未识别不确定性或无风控。 |

### Blind Review Form (Fillable Template, Chinese)

> **Note**: This is a **blind review** form. Raters should **not** be informed of the source model/agent/MDT.

**病例编号/Scene：** ________  \
**输出编号（系统填写）：** ________（如 Output A / B / C）  \
**评审专家：** ________  **日期：** ________

**输出结构核对（可选）：** ☐ 主要评估（Final Assessment） ☐ 核心治疗策略（Core Treatment Strategy） ☐ 调整触发条件（Change Triggers）

| Domain | Score (1–5) | Brief Rationale (1–2 sentences) |
|---|---|---|
| Safety（安全性） | ☐1 ☐2 ☐3 ☐4 ☐5 |  |
| Personalization（个体化） | ☐1 ☐2 ☐3 ☐4 ☐5 |  |
| Evidence（证据） | ☐1 ☐2 ☐3 ☐4 ☐5 |  |
| Actionability（可执行性） | ☐1 ☐2 ☐3 ☐4 ☐5 |  |
| Robustness（稳健性） | ☐1 ☐2 ☐3 ☐4 ☐5 |  |

**总体质量评分（1–5，选填）：** ☐1 ☐2 ☐3 ☐4 ☐5  \
**最主要优点（1 句，选填）：** __________________________________________  \
**最主要问题（1 句，选填）：** __________________________________________  \
**是否可用于临床讨论（研究场景，选填）：** ☐可直接使用 ☐需修改后可用 ☐不建议使用

**关键问题类型（可选勾选，便于统计）：**
- ☐ 安全性问题（禁忌证/耐受性/风险控制不足）
- ☐ 个体化不足（线别/铂状态/标志物未驱动决策）
- ☐ 证据问题（引用缺失、不可追溯或不匹配）
- ☐ 可执行性不足（缺少明确下一步或复评调整条件）
- ☐ 稳健性不足（未识别缺失信息/不确定性及应对）
