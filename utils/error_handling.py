...
from servers.provenance_tracking import TraceLogger
...
def get_fallback_response(role: str, stage: str) -> str:
    ...
    """
    stage_fallbacks = {
        "initial_opinion": f"[Generation unavailable: initial opinion not produced. Role: {role}]",
        "final_plan": f"[Generation unavailable: final plan not produced. Role: {role}]",
        "turn_speak": f"[Generation unavailable: deliberation turn not produced. Role: {role}]",
        "summary": f"[Generation unavailable: summary not produced. Role: {role}]",
        "memory_update": f"[Generation unavailable: memory update not produced. Role: {role}]",
    }
    
    return stage_fallbacks.get(stage, f"[Generation unavailable. Role: {role}, Stage: {stage}]")
...
def safe_agent_call(
    ...
    Args:
        ...
        use_once: If True, use chat_once() instead of stateful chat()
        retry_delay_seconds: Optional fixed delay between retries. If omitted,
    ...
    for attempt in range(max_retries + 1):
        try:
            if use_once:
                response = agent.chat_once(prompt, timeout=timeout)
            elif use_selection:
                response = agent.run_selection(prompt, timeout=timeout)
            else:
