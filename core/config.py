...
def _with_runtime_overrides(config: Dict[str, Any]) -> Dict[str, Any]:
    """Apply product runtime environment overrides without mutating cached config."""

    resolved = copy.deepcopy(config)
    explicit_trace_db = (
        os.getenv("OMGS_API_TRACE_DB", "").strip()
        or os.getenv("OMGS_MAIN_API_TRACE_DB", "").strip()
    )
    if explicit_trace_db:
        resolved.setdefault("output_dirs", {})["api_trace_db"] = explicit_trace_db
    return resolved
...
def load_paths_config(config_path: str = "config/paths.json") -> Dict[str, Any]:
    ...
    if _CONFIG_CACHE is not None:
        return _with_runtime_overrides(_CONFIG_CACHE)
    ...
    try:
        ...
        config = {
            ...
            "llm": {**default_config["llm"], **loaded_config.get("llm", {})},
        }
...
def create_question(sample: dict):
    ...
    - question_raw: original raw user query text, retained in memory only
    """
