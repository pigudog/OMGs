...
def openrouter_reasoning_extra_body(model_info: str, enable_reasoning: bool = False) -> Dict[str, Any]:
    """Return OpenRouter reasoning controls for an OMGs model call."""
    deployment = str(model_info or "").lower()
    # Gemini 3 thinking cannot be fully disabled; use the high/default-aligned effort.
    if "gemini-3" in deployment:
        return {"reasoning": {"effort": "high"}}
    # DeepSeek-R1 endpoints require provider-managed reasoning; disabling it can cause API errors.
    if "deepseek/deepseek-r1" in deployment:
        return {}
...
class Agent:
    ...
    def _request_extra_body(self) -> Optional[Dict[str, Any]]:
        """Return provider-specific extra request body, if needed."""
        if getattr(self.client, "provider", None) != "openrouter":
            return None
        return openrouter_reasoning_extra_body(self.deployment, self.enable_reasoning)
    ...
    def chat(self, message: str):
        ...
        try:
            ...
            resp = self.client.chat_completion(**request_kwargs)
    ...
    def chat_once(
        ...
        """
        Sends a stateless one-shot user message and returns assistant response.

        This is intended for reducer/synthesis calls whose continuity is carried
        by explicit prompt inputs rather than this agent's private chat history.
        """
        system_text = self.messages[0].get("content", self.instruction.strip()) if self.messages else self.instruction.strip()
        ...
        msgs = self._trim_messages_to_budget(msgs, int(max_prompt_tokens or self.max_prompt_tokens))
        ...
        try:
            ...
            raise AgentError(self.role, "chat_once", e)

    def inject_assistant(self, message: str):
        self.messages.append({"role": "assistant", "content": message})

    def run_selection(self, message: str, timeout: float | None = None):
        """
